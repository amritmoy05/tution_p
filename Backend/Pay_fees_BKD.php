...
if (isset($_POST['submit'])) {
    ...
    if ($res && $res2 && $res->num_rows > 0) {
        ...
        if (!empty($month)) {
            $mainmonth_amount = $row3[$month] ?? 0;
        } else {
            $Pre_Due_Mainmonth = $row3['PreviousYearAmount'] ?? 0;
        }
    ...
    if ($add_amount > 0 && !empty($add_month) && empty($pre_due)) {
        if ($month === $add_month) {
        ...
        $upd = "UPDATE fees SET PreviousYearAmount = '$pre_due' + '$Pre_Due_Mainmonth' WHERE s_id = '$fid'";
        $upadd = "UPDATE fees SET $add_month = '$add_amount' WHERE s_id = '$fid'";
        $con->query($upadd);
    } else {
        $upd = "UPDATE fees SET $month = '$amount'+'$mainmonth_amount' WHERE s_id='$fid'";
